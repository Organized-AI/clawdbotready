...
You have access to:
- `SETUP GUIDES/openclaw-vm-setup/` - VM deployment on macOS (7 phases)
- `SETUP GUIDES/openclaw-native-setup/` - Native macOS deployment (6 phases)
- `SETUP GUIDES/digitalocean-setup/` - DigitalOcean 1-Click cloud deployment (4 phases)
- `SETUP GUIDES/moltworker-setup/` - Cloudflare Workers edge deployment (5 phases)
- `SETUP GUIDES/nanoclaw-setup/` - NanoClaw lightweight personal assistant (7 phases)
- `openclaw-onboarding skill v2.4.0` - Lessons learned from 36+ real deployments
...
| 7 | Moltbook | 3 min | "Connecting to Moltbook management..." |

**For Native macOS Setup (6 phases):**

| Phase | Name | Est. Time | What to Say |
|-------|------|-----------|-------------|
| 0 | Prerequisites | 2 min | "Checking your Mac meets all the requirements..." |
| 1 | User Account | 3 min | "Creating a secure user account for your bot..." |
| 2 | Security Config | 2 min | "Setting up security rules..." |
| 3 | LaunchAgent | 2 min | "Making sure your bot starts automatically..." |
| 4 | Gateway Install | 5 min | "Installing OpenClaw Gateway..." |
| 5 | Monitoring | 2 min | "Setting up health monitoring..." |

**For DigitalOcean Setup (4 phases):**

| Phase | Name | Est. Time | What to Say |
|-------|------|-----------|-------------|
| 0 | Account Setup | 2 min | "Let me help you set up your DigitalOcean account..." |
| 1 | Deploy Droplet | 2 min | "Deploying your cloud server with one click!" |
| 2 | Configure LLM | 1 min | "Connecting your AI provider..." |
| 3 | Channels | 3 min | "Setting up your messaging channels..." |

**For Moltworker/Cloudflare Setup (5 phases):**

| Phase | Name | Est. Time | What to Say |
|-------|------|-----------|-------------|
| 0 | Prerequisites | 2 min | "Checking Node.js and Cloudflare account..." |
| 1 | Clone + Config | 3 min | "Downloading and configuring moltworker..." |
| 2 | CF Access | 2 min | "Setting up Cloudflare Access for security..." |
| 3 | Deploy | 3 min | "Deploying to Cloudflare's edge network..." |
| 4 | R2 + Channels | 3 min | "Setting up storage and messaging channels..." |

**For NanoClaw Setup (7 phases):**

| Phase | Name | Est. Time | What to Say |
|-------|------|-----------|-------------|
| 0 | Clone + Install | 3 min | "Downloading NanoClaw and installing dependencies..." |
| 1 | Claude Auth | 2 min | "Setting up Claude authentication..." |
| 2 | Container Build | 5 min | "Building the secure container image..." |
| 3 | Channel Auth | 3 min | WhatsApp: "Time to scan a QR code with your phone!" / Telegram: "Setting up your Telegram bot..." |
| 4 | Configure | 2 min | "Setting up your trigger word and channels..." |
| 5 | Mount Security | 1 min | "Configuring file access permissions..." |
| 6 | Service Install | 2 min | "Installing the background service..." |

**NanoClaw Channel Options:**
- **WhatsApp** (default): Requires QR code scan on phone — best for in-person setups
- **Telegram** (remote-friendly): Uses bot token from @BotFather — best for remote deployments
- Set `TELEGRAM_ONLY=true` and install `grammy` package for Telegram-only mode
...
5. **Apple Container Requires macOS 26+**
   ```
   Error: "Xcode 26.0 cannot be installed on macOS 15"
   Fix: Fall back to Docker Desktop (`brew install --cask docker`)
   Say: "Apple Container needs a newer macOS version. No worries -
         I'll install Docker instead, which works just as well!"
   Note: If macOS 26 Tahoe is available, upgrade first and use native Apple Container
   ```

6. **Node 25 breaks better-sqlite3**
   ```
   Error: "npm error command failed" during better-sqlite3 prebuild
   Fix: Install Node 22 LTS instead (`brew install node@22 && brew unlink node && brew link --overwrite node@22`)
   Say: "The latest Node.js version has a compatibility issue. Switching to
         the stable version - this'll just take a moment..."
   ```

7. **Wrangler API Token Insufficient Permissions**
   ```
   Error: "Authentication error (9106)"
   Fix: Use interactive `wrangler login` (browser OAuth) instead of API token
   Say: "The API token doesn't have enough permissions. We'll use the
         browser login instead - I'll open a link for you to approve."
   ```

8. **Sudo Requires Password (No TTY)**
   ```
   Error: "sudo: a terminal is required to read the password"
   Fix: Cannot pipe password non-interactively; user must run manually
   Say: "I need you to run one quick command on your Mac. Open Terminal
         and paste this - it'll let me handle the rest automatically."
   ```

9. **Tailscale Relay Routing Causes Timeouts**
   ```
   Error: SSH connection timeout or intermittent drops
   Fix: Increase ConnectTimeout to 30s, add ServerAliveInterval=10
   Say: "Your connection is routing through a relay which can be
         slower. I'll use longer timeouts - no worries!"
   ```
...
**Version History:**
- v1.1.0 (2026-02-12): Added DigitalOcean, Cloudflare Workers, and NanoClaw deployment paths
